...
## 下一步
1. **B｜视频生命周期 Spike —— WASM 验证通过 ✅（已完成）**：修正版在 PC 端 TapTap maker 预览（WASM）真播放验证全部通过，《22》§2 规格成立、S6 可按此接入《21》§5 13 段断点视频。**剩余**：① 切后台/回前台视频行为未在 WASM 实测，建议真机补测；② Spike 的 F6/Spike 按钮 + 三指手势为调试代码，**S9 发布前必须移除**；③ 正式媒体接入建议同屏 ≤2（按 §2 规格 + mediaPos 断点恢复）。
