...
项目：桃素洛无幽·素女篇
当前版本：v0.3.14-avatar-shell
简述：TapTap 制造 × Seedance 主题赛单机叙事游戏。本轮：三选头像作为 StoryPanel shell 子节点，固定在底板右下空白区（right 3%、bottom 8%、100×100），不再位于屏幕中部悬浮层；三选选项、字幕和头像同属底板层级。
最后巩固：2026-09-03
...
## 最近变更
- 2026-09-03 **三选头像移入底板面板（本会话）**：`DialogueUI.RenderChoice` 创建头像后调用 `shell:AddChild(portrait)`，头像与三选按钮同属 `StoryPanel` shell；定位为 `right="3%"`、`bottom="8%"`、`100×100`，保留圆角、金边和 `pointerEvents="none"`，不再挂在全屏 panel 上。LSP 0 Error、官方构建成功、60 帧验证 `lua_errors=0`，未出现新的 Lua runtime error；真机截图仍待补验。
...
- 2026-08-30 **素材 Phase 1：S6-1..5 + S7/S8/S9 正式视频接入（本会话）**：Seedance 2.0 / 15s / 9:16 / 720p / 有声，定妆图（素女/无幽）作参考。落盘 `assets/video/剧情/`：S6-1_礼、S6-2_乐、S6-3_射、S6-4_御、S6-5_书数、S7_圆满、S8_放手、S9_传说。`VIDEO_SOURCES` 替换占位；P32~P36 断点 `at=8.5`（关键画面约 8~10s 前 0.4s，真机再校）；P43/44/45 补 `at=-1/auto`。S1/S2~S4/S5 仍占位。LSP 0 Error、官方构建成功、140 帧 lua_errors=0。⚠️ 真机：S6链看片校正 at、三选锁定、结局卡。
- 2026-08-30 **ch2/ch4 场景填充（本会话）**：只做加法。洛水阴山：雾更冷更近（`fogStart=18`/`fogEnd=70`/`fogDensity=1.2`，冷蓝主光 0.62；**不用高度雾**，第三人称相机约 6.8m 会被近雾整屏吃掉）；加 TownHouse4/5 + 屋棚、山泉井（SpringWell/Frame/Posts + 蓝光柱）、无面鬼暗红/冷色双光柱 + 点光、矿场小路岩石+3 盏矮灯。朝阳谷口终局：无涕桃周围 8 块矮石板+粉白光柱+柔和点光；守桃老人旁暖色矮灯/光柱/点光。**未改** WutiTao/Int_oldman/Beacon_oldman/五朵桃花/出生点/边界墙。F2/F3/F4 直切场景时关主菜单并对准地标。LSP 0 Error、官方构建成功、140 帧 lua_errors=0；三场景截图已出（朝阳谷口无涕桃圈、谷内桃林屋井、洛水阴山无面鬼石台/光柱/矿灯）。
- 2026-08-30 **整条主链真机验收出码（8910045）**：静态主链完整：P01→P02→P03→P04(S2)→P05(S3)→P06(S4)→P07→P11→P12→P21(S5)→P22(resolveNext→P23/24/25)→P31→P32~P36→P41→P42(resolveNext→P43/44/45)→P99(end)→结局卡→主菜单。存档：右上「菜单」SaveMenu.Persist；读档 Resume；启动「等待主菜单选择开始/继续」不再自动续档。LSP 0 Error、官方构建成功、140 帧 lua_errors=0（启动主菜单+EndingScreen+SaveMenu，不进 P01）。二维码已生成，⚠️ 待真机整链走通 + 存档/读档/重启续档。
- 2026-08-30 **正式结局画面 + 制作名单（本会话）**：新增 `scripts/ui/EndingScreen.lua`（全屏遮罩+居中卡片，样式同 MainMenu；一次建卡，`Show(endingKey)` 只用 `SetText`/`SetVisible` 换文案）。三结局：`round`「圆满 · 桃花又香」/`release`「放手 · 无泪无悔」/`legend`「传说 · 会有新的旅人」+ 制作名单 +「返回主菜单」。`Chapters` P43/P44/P45 加 `ending` 字段；`FlowController` 记 `pendingEnding_`，`end` 段回调 `onGameEnd_(pendingEnding_)`；`Init` 清残留结局键。`main` 创建 EndingScreen、`SetOnGameEnd→Show`、`SetOnReturn→MainMenu.Show`，`HandleUpdate` 结局卡打开时锁移动。MainMenu 开始/继续先 `EndingScreen.Close()`。LSP 0 Error、官方构建成功、140 帧 `lua_errors=0`（启动 `[EndingScreen] 已创建结局画面` + 主菜单，不进 P01）。二维码已生成，⚠️ 待真机：S6链→结局视频→弹出对应结局卡→返回主菜单、三种结局各自对上文案、0 崩溃。
- 2026-08-30 **结局返回主菜单真机通过 ✅（`break.log` 01:32 定案）**：`S6链` 直切 → P31→P32~P36（S6-1..5 ENDED）→P41→P42→**P43(S7 结局)**→**P99(end)**→ **`[Flow] 结局段落（返回主菜单）`**，**未再循环回 P01**，**0 崩溃**。✅ **正式结局 + 主菜单可玩闭环定案**；下一阶段主线候选：正式素材替换 / ch2·ch4 场景填充 / 正式结局画面·制作名单替换。
- 2026-08-30 **结局返回主菜单快速补测核对 + 构建出码（3eff7d3）**：路径核对 P31→P36→P41→P42→P43/44/45→P99(end)→`onGameEnd`→`MainMenu.Show()`，不再循环 P01。**TTM 补丁**（rebase 合入 `a2d0522` 定案之后）：① `MainMenu.Show()` 打 `[MainMenu] 显示主菜单（有/无存档）`；② S6链触屏与 F10 直切前 `MainMenu.Close()`，避免主菜单全屏遮罩挡住 P31。LSP 0 Error、官方构建成功、140 帧 `lua_errors=0`。真机闭环已由 `break.log` 01:32 定案。
- 2026-08-30 **主菜单真机验证 ✅（`break.log` 01:19）**：两次启动均 `[MainMenu] 已创建主菜单` + `检测到本地存档，等待主菜单选择开始/继续`（不再自动续档/进 P01）；「开始游戏」→ `[PlayerData] 已删除存档` + 全新 `进入段落 P01`（跑完 P01→P11）；「继续游戏」→ `[Flow] 读档恢复：定位段落 P02`；**0 崩溃**。✅ 主菜单（创建/开始清档进 P01/继续读档/0 崩溃）**定案**。⚠️ **结局返回主菜单**：本次未运行到结局（停 P11/P02），代码侧核对通过（`f2482ed`/`36b2591`：P43/44/45→P99(end)→`onGameEnd`→`MainMenu.Show()`），**待补一次"运行到结局"真机**（预期 `[Flow] 结局段落（返回主菜单）`→主菜单再出现）。
- 2026-08-30 **正式主菜单 + 结局返回核对 + 构建出码（f2482ed/f47ecb2）**：新增 `scripts/ui/MainMenu.lua`（标题「桃素洛无幽 · 素女篇」+ 副标题 + 「开始游戏」/「继续游戏」；`Show()` 按 `PlayerData.Load()` 决定继续按钮可见/可点）。启动不再自动续档，由玩家选择。`FlowController` `end` 段改为触发 `onGameEnd_`（main 注入 `MainMenu.Show`），P43/P44/P45 播完进 P99 返回主菜单，**不再循环回 P01**。`PlayerData.Clear()` 删 `saves/slot1.json` 并返回全新默认数据，`doStart` 用这份数据 `Init+Start`（belief/blossoms 重置）。`HandleUpdate` 对 `MainMenu.IsOpen()` 调 `ClearMovement()` 锁移动。**TTM 补丁**：① `onStartGame_` 原标 `table\|nil` 导致 LSP Error（`SetOnStart` 赋 function），改为 `function\|nil`；② `doStart` 先 `MediaPlayer.Stop(true)` 防结局视频残留串台；③ 启动日志「尝试自动续档」改为「等待主菜单选择开始/继续」，避免真机误判。核对：`fileSystem:Delete` 存在（`.emmylua/FileSystem.d.lua`）；无档时继续按钮 `SetVisible(false)+SetDisabled(true)`；`EnterParagraph` 进新段会 `PlayerData.Save`，故结局返回后「继续」可读档到 P99。LSP 0 Error、官方构建成功、140 帧 `lua_errors=0`（启动日志：`[MainMenu] 已创建主菜单`，**不进 P01**）。二维码已生成，⚠️ 待真机验证：主菜单显示/开始清档进 P01/有档可续/结局返回主菜单/锁移动/无崩溃。
- 2026-08-30 **P04~P06 讲述段回忆视频整合真机通过 ✅（`break.log` 00:16 定案）**：新开一局自然链 P02→P03→P04→P05→P06→P07→P11；P04 `[Flow] 讲述段 P04 先播回忆视频 S2`→`ENDED`→`[Flow] 对话段 P04（legend_part1）`，P05(S3→legend_part2)、P06(S4→legend_part3) 依次，最终推进到 **P07（接任务）**，**0 崩溃**。✅ **P04~P06 回忆视频整合闭环**；下一阶段主线候选：正式素材替换 / 正式结局菜单 / ch2·ch4 场景填充。
- 2026-08-29 **P04~P06 讲述段回忆视频整合核对 + 构建出码（568a0a6，用户提交）**：`FlowController` 新增 `pendingDialogueAfterVideo_`——`dialogue` 且带 `video` 字段（P04~P06）先合成 `{type="video", breakpoints={{at=-1,act="auto"}}}` 交给 `MediaPlayer.Play` 播回忆视频（S2/S3/S4 占位），`completeHandler` 检测到 pending 则 `StartDialogueParagraph` 弹该段讲述对白（`legend_part1~3`），否则按正常收尾推进。**核对结论**：① 闭包内 `StartDialogueParagraph`/`CompleteParagraph` 是前向 local，但回调运行期才执行（届时函数已定义），安全 ✅；② `MediaPlayer.Stop(true)` 不触发回调，无 pending 残留（且 EnterParagraph 每次进讲述段都重新赋值）✅；③ `checkBreakpoint` 要求 `at>=0`，合成段 `at=-1` 永不触发 ✅；④ 对白 `onDone→CompleteParagraph` 正常推进（P04→P05→P06→P07）✅；⑤ 读档命中讲述段时 video 优先、播完进对白，属可接受行为。LSP 0 Error、官方构建成功、140 帧 `lua_errors=0`（字体缺失/无解码器为既有环境噪音）。二维码已生成，⚠️ 待真机验证：P03 三选后 P04 先播 S2 → `[Flow] 对话段 P04（legend_part1）` → … → P06 后推进 P07。
- 2026-08-29 **存档/读档菜单真机通过 ✅（`break.log` 22:11 定案）**：菜单创建、点「保存」→ `[SaveMenu] 已保存` + `[PlayerData] 已保存到 saves/slot1.json`（×11/×13）、点「读档」→ `[SaveMenu] 已读档并续播` + `[Flow] 读档恢复：定位段落 P11`、重启 `检测到本地存档，尝试自动续档` → 续到 P11、`读档成功 mediaPos.node=P11`，**GetVisible/崩溃=0**（`e59e4a0` 修复生效）。✅ **存档/读档菜单定案**；下一阶段主线候选：正式素材替换 / P04~P06 对话+视频整合 / 正式结局菜单。
- 2026-08-27 **SaveMenu 核对 + GetVisible 崩溃修复 + 构建出码（e59e4a0，基于 4fb45c1/f3e60fe）**：新增 `scripts/ui/SaveMenu.lua`（右上"菜单"按钮 + 居中存档面板：状态行/保存/读档/返回，复用 PlayerData.Save/Load + FlowController.Persist/Resume），main 对话/菜单打开均 `ClearMovement()`。**阻断 bug 修复**：`IsOpen()` 原调不存在的 `panel_:GetVisible()`（UI 库只有 `IsVisible()`/`SetVisible()`），main 首帧 HandleUpdate 即 nil-call——改为 `panel_:IsVisible()`；`variant="ghost"` 库无此变体但优雅回退默认色（仅外观）。另：采集/交互玩法闭环 fccfc8b 已真机全通过（333a1f1）。本地 LSP 0 Error、官方构建成功、140 帧 `Lua runtime error=0`；二维码已生成待真机验证（菜单开关/保存/读档续播/锁移动/重启自动续档）。
- 2026-08-27 **采集边沿触发核对 + 构建出码（7e98ac5，含 177170e/fccfc8b）**：轮询改"进入半径"边沿触发（`inRadius_` 按节点记录，离开重置，防每帧重复上报）；仅 explore 段启用（`EnterParagraph` 调 `SetPickupsEnabled(p.type=='explore')`，非探索段不刷屏）；`OnBlossomCollected` 加 `无独白映射`/`独白数据缺失` 兜底日志（含 `p`/`type`/`monoKey`/`hasMonologue`）用于定位 wood 独白。本地 LSP 0 Error、官方构建成功、140 帧 `Lua runtime error=0`；二维码已生成待真机重测（每朵一次无重复、wood 弹独白、非 explore 不刷屏、5/5 进 P12）。
- 2026-08-27 **采集/交互玩法本阶段定案（`fccfc8b` 真机 06:56 全通过）**：真机 `break.log`(06:52-06:56) 验证边沿触发修复——拾取 earth/fire/metal/water/wood/oldman **各恰 1 次无重复**、**5 朵独白全弹（含 wood）**、`桃花进度 5/5→P12`、`走近交互 P02`、主链 →**P43(S7 圆满)**→P99、信念 6/4/5→重逢、**0 崩溃**、无兜底日志。✅ **采集/交互玩法闭环**；下一阶段：**存档 UI/菜单**（把 F8/F9 做成正式存档界面）。
- 2026-08-27 **采集稳健化核对 + 构建出码（c62fb75，含 177170e）**：修复对话中拾取丢失(wood 独白未现)与重复 `拾取标记`——轮询只上报命中，标记/光柱移除移到 `FlowController.OnBlossomCollected(key, node)` 通过 `DialogueUI.IsOpen()` 守卫后执行（对话中不消费，节点保留待下帧）；交互分支(P02)在移除逻辑前 return，旧 `Int_oldman` 由 P11 `LoadScene` 重建清理；`node.parent` 判空防御。PC 无碰撞订阅无双触发。本地 LSP 0 Error、官方构建成功、140 帧 `Lua runtime error=0`；二维码已生成待真机重测（每朵含 wood 都弹独白、无重复拾取、光柱随采消失、5/5 进 P12）。
- 2026-08-27 **拾取轮询修复核对 + 构建出码（b610a77，含 5e2ea8f）**：真机根因 = KCC 运动学角色不触发全局 `PhysicsCollisionStart`，拾取从未发生。修复 = `PlayerController.Update` 距离轮询（`PICKUP_RADIUS=1.1m`，识别 `Blossom_<五行>`/`Int_<key>`，命中即移除标记+联动移除 `Beacon_<key>` 光柱，`break` 一次只收一个）；`Create()` 已不再订阅碰撞事件（PC 无双触发）；对话期 `PlayerController.Update` 不执行 → 轮询暂停，`DialogueUI.IsOpen()` 双保险仍在。命名核对一致（`Beacon_<五行>`/`Beacon_oldman`）。本地 LSP 0 Error、官方构建成功、140 帧 `Lua runtime error=0`；二维码已生成待真机重测 P02/P11 自然拾取与光柱随采消失。
- 2026-08-27 **白模辨识度核对 + 构建出码（39659ab，含 0a955d7/9ad228f）**：`WhiteBox.Beacon`（细长自发光圆柱）接入 `BlossomMarker`（五行桃花各一根彩色光柱）+ 朝阳谷口守桃老人（金黄 2.4m 高光柱）；`SCENE_MOOD` 加 `name`、`SetOnSceneLoaded` 回调回传场景名；main 顶部居中横幅 `「场景名」`（pointerEvents=none，创建先于 FlowController.Start 不错过首场景回调，随根在视频会话接管/恢复时保留）。本地 LSP 0 Error、官方构建成功、140 帧 `Lua runtime error=0`；二维码已生成待真机验证（光柱可见、横幅随切换、摇杆/触摸视角、P02/P11 自然走通）。
- 2026-08-27 **白模辨识度：桃花/守桃老人采集光柱 + 场景名横幅（本会话）**：真机反馈白模下分辨不出哪个是桃花/守桃老人/当前场景。`WhiteBox` 加 `Beacon`（细长自发光圆柱），`BlossomMarker` 自动加光柱（桃花=醒目可采撷光柱）；`SceneManager` 朝阳谷口守桃老人加辨识光柱（走近交互），`SCENE_MOOD` 加 `name` + `SetOnSceneLoaded` 回调，`LoadScene` 回传场景名；`main.lua` 顶部场景名横幅（「朝阳谷口/谷内桃林/洛水阴山」）随场景切换更新。LSP 0 新增 error（提交 `9ad228f`）。⚠️ 需真机验证光柱可见、横幅随场景正确切换。
