...
```text
command: npx
args: ["-y", "-p", "@taptap/maker@0.0.30", "taptap-maker"]
```
...
```bat
"<绝对 node.exe>" "<绝对 npm-cli.js>" exec --yes --package @taptap/maker@0.0.30 -- taptap-maker
```
...
```bash
npx -y -p @taptap/maker@0.0.30 taptap-maker help
```
...
```bash
npx -y -p @taptap/maker@0.0.30 taptap-maker mcp install --launcher self
```
